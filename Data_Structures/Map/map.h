...
void putItem(map* m, void* key, void* data); //Used to put a new Key-Value pair into the Map
...
int containsKey(map* m, void* key); //Checks if a Key is present in the Map

//Internal Functions
map_block* _newMapBlock(); //Create a new Map Block
void _cleanMap(map *m); //Destroy unused Map Blocks
map_block* _findBlock(map* m, void* key); //Returns the Map Block containing the given Key
...
map_block* _findBlock(map* m, void* key)
{
	map_block* cur;
	int k;
...
int containsKey(map* m, void* key)
{
	return _findBlock(m,key)!=NULL;
}
void putItem(map* m, void* key, void* data)
{
	int k = m->genKey(key);
	
	map_block* _found = _findBlock(m,key);
	if(_found)
	{
		m->cpyData(_found->block[k].data, data);
		return;
	}
